...
try {
  ...
  // 7. A malformed provider response fails closed as a failed run (no dispatch).
  {
    ...
    assert(!!msgEnd && typeof msgEnd.error === "string" && msgEnd.error.length > 0,
  ...
  // error indication before the run ends as failed.
  {
    ...
    const msgEnd = result.events.find((event) => event.type === "message_end");
    assert(msgEnd.error === "boom", "message_end carries the provider error");
  }
  ...
  // an outside directory — the trusted loop root always wins.
  {
    ...
    assert(readResult.result.content.includes("generated by **OAF**"),
      "read used the trusted workspace root, not the attacker-supplied root");
    assert(!String(readResult.result.content).includes("EVIL README CONTENT"),
      "attacker workspaceRoot was ignored (outside file never read)");
  ...
  // executor dispatch.
  {
    ...
    assert(!!errA && errA.error.includes("unexpected argument"), "unexpected key rejected with an error result");
    ...
    assert(!!errB && errB.error.includes("non-null object"), "non-object args rejected with an error result");
  }
} finally {
