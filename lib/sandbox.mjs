...
import { BLESSED_PACKAGE_MANAGER, BLESSED_PACKAGE_SCRIPTS, canonicalCommand, isGitInspectionCommand, isVerificationCommand } from "./command-policy.mjs";
import { COMMAND_ORIGINS, COMMAND_POLICY_LEVELS, SANDBOX_MODES as SANDBOX_MODE_VALUES } from "./agent/contracts.ts";
...
export async function verifyPackageScript(workspaceRoot, command) {
  ...
  if (existsSync(resolve(workspaceRoot, "pnpm-workspace.yaml"))) {
    policyFailure("pnpm-workspace.yaml is not OAF-owned");
  }
  if (existsSync(resolve(workspaceRoot, ".pnpmfile.mjs")) || existsSync(resolve(workspaceRoot, ".pnpmfile.cjs"))) {
...
function spawnContainer({ runtime, argv, onStart, onStdout, onStderr, command, network, cwd }) {
  ...
  return new Promise((resolvePromise, rejectPromise) => {
    ...
    child.stderr?.on("data", (data) => { stderr += data; onStderr?.(data); });
...
async function executeSandbox({ command, mode, origin, approvalGranted, networkGranted, cwd = process.cwd(), onStart, onStdout, onStderr, dependencies = {} }) {
  ...
  if (origin === "agent" && normalized === "git diff") {
    throw new SandboxError("AGENT_COMMAND_DENIED", "agent command is not available to the provider");
  }
  ...
  try {
    ...
    const options = { runtime, argv, onStart, onStdout, onStderr, command: normalized, network: origin === "agent" ? false : (networkGranted || verdict.network), cwd: executionRoot };
    return await (dependencies.runContainer ?? spawnContainer)(options);
  } finally {
...
/** @param {import("./agent/contracts.ts").AgentSandboxCommandOptions} [options] @returns {Promise<import("./agent/contracts.ts").SandboxExecutionResult>} */
export async function runAgentSandboxCommand(options = {}) {
  if (options === null || typeof options !== "object" || Array.isArray(options)) {
    throw new SandboxError("INVALID_AGENT_ARGUMENT", "agent sandbox options must be an object");
  }
  const allowed = new Set(["command", "mode", "cwd", "onStart", "onStdout", "onStderr", "dependencies"]);
  for (const key of Object.keys(options)) {
    if (!allowed.has(key)) throw new SandboxError("INVALID_AGENT_ARGUMENT", `agent sandbox received unexpected argument: ${key}`);
  }
...
export async function runHumanSandboxCommand({ command, mode, approvalGranted = false, networkGranted = false, cwd, onStart, onStdout, onStderr, dependencies } = {}) {
