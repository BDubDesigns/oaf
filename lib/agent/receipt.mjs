...
import { MAX_MODEL_IDENTIFIER_LENGTH, MAX_PROVIDER_IDENTIFIER_LENGTH, normalizeProviderIdentifier } from "./provider.mjs";
...
export class ReceiptWriteError extends Error {
  constructor() { super("receipt could not be written"); this.code = "RECEIPT_WRITE_FAILED"; }
}
...
export async function runAgentLoopWithReceipt({
  ...
  if (receiptUsage) receipt.usage = validateReceiptUsage(typeof receiptUsage === "function" ? receiptUsage(run) : receiptUsage);
  let receiptPath;
  try { receiptPath = await writeReceipt({ workspaceRoot, receipt }); }
  catch { throw new ReceiptWriteError(); }
