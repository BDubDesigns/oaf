...
export class ReceiptWriteError extends Error {
  constructor(run, receipt) { super("receipt could not be written"); this.code = "RECEIPT_WRITE_FAILED"; this.run = run; this.receipt = receipt; }
}
...
export async function runAgentLoopWithReceipt({
  ...
  catch { throw new ReceiptWriteError(run, receipt); }
