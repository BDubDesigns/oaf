...
import { recordContinuation } from "./events.mjs";
...
// (already redacted/omitted) command string. Status comes from the tool result.
...
// Variable/option names that signal a secret. Hyphens are normalized to
// underscores before matching so `--api-key` and `API_KEY` are treated alike.
...
// Scan a command and decide whether it is too suspicious to sanitize with
// confidence. We deliberately favor omission: a suspicious command is replaced
// wholesale by `<redacted command>` rather than partially redacted.
function commandLooksSuspicious(command) {
  if (typeof command !== "string" || command.length === 0) return false;
  const normalized = command.replace(/-/g, "_");
  if (SECRET_KEYWORD_RE.test(normalized)) return true;
  if (/authorization:/i.test(command)) return true;
  if (/\:\/\/[^\s:/]+:[^\s@]+@/.test(command)) return true;
  return false;
}

// Redact a command: if it looks suspicious, omit it entirely; otherwise keep
// it verbatim (known-safe commands like `pnpm test` stay visible).
function redactCommand(command) {
  if (commandLooksSuspicious(command)) {
    return { command: "<redacted command>", redacted: true };
  }
...
export function buildReceipt({ run, task, oafVersion = OAF_VERSION }) {
  ...
  const status = terminalSucceeded ? (partial ? "partial" : "success") : "failed";
  ...
  const nextSteps = status === "success"
