...
export const CANONICAL_COMMANDS = [
  { command: "pnpm test", name: "test", type: "test" },
  { command: "pnpm lint", name: "lint", type: "lint" },
  { command: "pnpm typecheck", name: "typecheck", type: "typecheck" },
  { command: "pnpm build", name: "build", type: "build" },
  { command: "git status", name: "vcs-status", type: "vcs" },
  { command: "git diff", name: "vcs-diff", type: "vcs" },
  { command: "git log --oneline", name: "vcs-log", type: "vcs" },
];

const CANONICAL_COMMAND_SET = new Set(CANONICAL_COMMANDS.map((c) => c.command));
...
export function safeCount(value) {
  if (!Number.isInteger(value) || value < 0 || value > MAX_COUNT) return null;
  return value;
}
