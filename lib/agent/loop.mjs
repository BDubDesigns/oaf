  ...
  while (turn < maxTurns) {
    ...
    for (let index = 0; index < requested.length; index++) {
      ...
      try {
        ...
        collector.record(createEvent("tool_result", { toolCallId: auditId, toolName, summary: summarizeToolResult(toolName, result), errorCode: null }));
        toolResults.push({ toolCallId: providerId, toolName, result });
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
