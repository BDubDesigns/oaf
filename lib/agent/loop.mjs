...
export async function runAgentLoop({
  ...
  while (turn < maxTurns) {
    ...
    for (let index = 0; index < requested.length; index++) {
      ...
      collector.record(createEvent("tool_call", { toolCallId, toolName: auditToolName, summary: summarizeToolCall(auditToolName, call?.args) }));
      ...
      if (!toolName || typeof toolName !== "string" || !TOOLS[toolName]) {
        ...
        collector.record(createEvent("tool_result", { toolCallId, toolName: auditToolName, summary: {}, errorCode: "rejected" }));
      ...
      try {
        ...
        const message = error instanceof Error ? error.message : String(error);
        collector.record(createEvent("tool_result", { toolCallId, toolName, summary: {}, errorCode: "rejected" }));
      ...
      collector.record(createEvent("tool_execution_start", { toolCallId, toolName }));
      try {
        const result = await executor({ ...validated, workspaceRoot });
        collector.record(createEvent("tool_execution_end", { toolCallId, toolName, success: true }));
        collector.record(createEvent("tool_result", { toolCallId, toolName, summary: summarizeToolResult(toolName, result), errorCode: null }));
        toolResults.push({ toolCallId, toolName, result });
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        collector.record(createEvent("tool_execution_end", { toolCallId, toolName, success: false }));
        collector.record(createEvent("tool_result", { toolCallId, toolName, summary: {}, errorCode: "execution_error" }));
        toolResults.push({ toolCallId, toolName, error: message });
      }
    }
