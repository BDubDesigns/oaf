...
/** @param {Omit<import("./contracts.ts").AgentLoopOptions, "task"> & { task?: unknown }} options */
export async function runAgentLoop({
  ...
  while (turn < maxTurns) {
    ...
    // Prior provider messages remain opaque to the loop until its runtime
    // validators dispatch them; this is the typed adapter boundary.
    const request = /** @type {import("./contracts.ts").NormalizedProviderRequest} */ ({ system, messages, tools });
    ...
    for (let index = 0; index < requested.length; index++) {
      ...
      let executorError;
      try {
        // The selected executor is dynamically keyed by the validated registry
        // entry, so its public boundary remains unknown until it returns.
        executorResult = await /** @type {(input: unknown) => Promise<unknown>} */ (executor)({ ...validated, workspaceRoot });
      } catch (error) {
