...
import { buildToolProtocol, ProviderFailure, validateProviderCall } from "./provider.mjs";
...
export async function runAgentLoop({
  ...
  while (turn < maxTurns) {
    ...
    try {
      ...
      providerAttempts.push({ turn, durationMs: Math.max(0, Date.now() - startedAt), outcome: failure.outcome, httpStatus: failure.httpStatus });
    ...
    try {
      ...
      providerAttempts.push({ turn, durationMs: Math.max(0, Date.now() - startedAt), outcome: "invalid_response", httpStatus: null });
      collector.record(createEvent("message_end", { turn, disposition: "provider_error", contentPresent: false, contentBytes: 0, toolCallCount: 0, errorCode: "provider_error" }));
      collector.record(createEvent("agent_end", { runId: run, status: "failed", turns: turn, terminalReason: "provider_error" }));
      return {
        runId: run,
