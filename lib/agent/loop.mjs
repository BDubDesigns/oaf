...
function providerOutcome(error) {
  const message = error instanceof Error ? error.message : "";
  const status = /HTTP status (\d{3})/.exec(message)?.[1];
  const httpStatus = status ? Number(status) : null;
  if (httpStatus === 401 || httpStatus === 403) return { outcome: "authentication_failed", httpStatus };
  if (httpStatus === 404) return { outcome: "not_found", httpStatus };
  if (httpStatus === 429) return { outcome: "rate_limited", httpStatus };
  if (httpStatus !== null) return { outcome: "http_error", httpStatus };
  if (/timed out/.test(message)) return { outcome: "timeout", httpStatus: null };
  if (/invalid JSON/.test(message)) return { outcome: "invalid_json", httpStatus: null };
  if (/exceeded.*size/.test(message)) return { outcome: "response_too_large", httpStatus: null };
  if (/malformed|invalid response|no choices|finish_reason/.test(message)) return { outcome: "invalid_response", httpStatus: null };
  if (/request failed/.test(message)) return { outcome: "network_error", httpStatus: null };
  return { outcome: "unknown_provider_error", httpStatus: null };
}
...
export async function runAgentLoop({
  ...
  while (turn < maxTurns) {
    ...
    try {
      ...
      providerAttempts.push({ turn, durationMs: Math.max(0, Date.now() - startedAt), outcome: "success", httpStatus: null });
    } catch (error) {
      providerAttempts.push({ turn, durationMs: Math.max(0, Date.now() - startedAt), ...providerOutcome(error) });
    ...
    for (let index = 0; index < requested.length; index++) {
      ...
      try {
        ...
        toolResults.push({ toolCallId: providerId, toolName, error: publicError.message, errorCode: publicError.code });
        continue;
      }
      ...
      try {
        ...
        executorError = publicToolError(error, toolName);
      }
      ...
      if (executorError !== undefined) {
        ...
        toolResults.push({ toolCallId: providerId, toolName, error: executorError.message, errorCode: executorError.code });
      } else {
