...
import { runSandboxCommand, SANDBOX_MODES } from "../sandbox.mjs";
...
export function createCommandExecutor({ sandboxRunner = runSandboxCommand } = {}) {
  if (typeof sandboxRunner !== "function") {
    throw new Error("sandboxRunner must be a function");
  }

  return async function executeCommand({ workspaceRoot, command, mode, network = false, confirm = false }) {
    const root = await resolveWorkspaceRoot(workspaceRoot);
    ...
    if (typeof network !== "boolean") {
      throw new Error("network must be a boolean");
    }
