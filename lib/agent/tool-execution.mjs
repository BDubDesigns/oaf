...
function validateProjectPath(path) {
  if (typeof path !== "string" || path.length === 0) {
    throw new AgentToolError("INVALID_TOOL_ARGUMENTS");
  }
  if (isAbsolute(path) || win32.isAbsolute(path)) {
    throw new AgentToolError("PATH_OUTSIDE_WORKSPACE");
  }
...
async function normalizeRequestedPath(workspaceRoot, path) {
...
async function resolveWriteDestination(workspaceRoot, path) {
  ...
  const resolvedRelative = outputPath(root, destination);

  return { root, parent, destination, path: requestedRelative, resolvedRelative };
}
...
function validateLineNumber(value, name) {
  if (value !== undefined && (!Number.isInteger(value) || value < 1)) {
    throw new AgentToolError("INVALID_LINE_RANGE");
  }
}
...
/** @type {(input: import("./contracts.ts").ToolExecutorInput<"read">) => Promise<import("./contracts.ts").ToolExecutorResults["read"]>} */
export async function executeRead({ workspaceRoot, path, startLine, endLine }) {
  ...
  let resolved;
  try { resolved = await resolveWorkspacePath(workspaceRoot, path); }
  catch (error) { if (error?.code === "ENOENT") throw new AgentToolError("PATH_NOT_FOUND", error); throw error; }
...
/** @type {(input: import("./contracts.ts").ToolExecutorInput<"write">) => Promise<import("./contracts.ts").ToolExecutorResults["write"]>} */
...
/** @type {(options?: { sandboxRunner?: typeof runAgentSandboxCommand }) => import("./contracts.ts").ToolExecutorMap["command"]} */
...
/** @type {import("./contracts.ts").ToolExecutorMap["command"]} */
...
/** @type {(input: import("./contracts.ts").ToolExecutorInput<"list">) => Promise<import("./contracts.ts").ToolExecutorResults["list"]>} */
...
// Search a regular file or every UTF-8 text file below a directory for a plain substring. `glob`, when supplied, uses
...
/** @type {(input: import("./contracts.ts").ToolExecutorInput<"grep">) => Promise<import("./contracts.ts").ToolExecutorResults["grep"]>} */
