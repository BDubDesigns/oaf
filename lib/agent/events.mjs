...
/** @typedef {import("./contracts.ts").AgentEvent} AgentEvent */
...
const DISPOSITIONS = new Set(EVENT_DISPOSITIONS);
const ERROR_CODES = new Set(EVENT_ERROR_CODES);
const STATUSES = new Set(RUN_TERMINALS.map(({ status }) => status));
const TERMINAL_REASONS = new Set(RUN_TERMINALS.map(({ terminalReason }) => terminalReason));
...
function summary(toolName, fields, result = false) {
  ...
  const allowed = result
    ...
    : ({ read: ["path"], list: ["path", "recursive"], grep: ["path"], write: ["path", "bytes"], command: ["command", "redacted", "mode"] }[toolName] ?? []);
