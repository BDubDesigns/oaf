...
async function resolveDirectory(path, label) {
  if (typeof path !== "string" || path.length === 0) {
...
async function resolveOwnedDirectory(root, path, label) {
...
async function readOwnedFile(root, path, label, required = true) {
  assertSafeRelativePath(path, label);
  const requested = resolve(root, path);
  if (!isInside(root, requested)) {
    throw new Error(`${label} resolves outside its owner root`);
...
function validateWorkspaceMarkers(app, stack, docsPack) {
...
function validateDocsPackId(id) {
...
function validateManifest(manifest, docsPackId, oafStack) {
...
function asContextDocument(source, document) {
...
export async function loadAgentContext({ workspaceRoot, oafRoot = DEFAULT_OAF_ROOT }) {
