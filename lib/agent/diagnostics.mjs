...
function toolEvents(events) {
  ...
  return [...calls.values()].map(({ toolName, outcome }) => ({ toolName, outcome: outcome ?? "unknown_provider_error" }));
}

export function buildDiagnostic({ run, usage, receiptPath }) {
  return {
    schemaVersion: "0.1.0",
    ...
    providerAttempts: Array.from({ length: run.turns }, (_, index) => ({ turn: index + 1, durationMs: null, outcome: run.terminalReason === "provider_error" && index + 1 === run.turns ? "unknown_provider_error" : "success", httpStatus: null })),
