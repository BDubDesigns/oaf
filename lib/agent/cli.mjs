...
export async function runAgentCli({ taskParts, cwd = process.cwd(), oafRoot = resolve(import.meta.dirname, "..", ".."), env = process.env, output = console } = {}) {
  ...
  try {
    ...
    if (error?.code === "RECEIPT_WRITE_FAILED" && env.OAF_DIAGNOSTICS === "1" && error.run) {
      try {
        const path = await writeDiagnostic({ workspaceRoot: cwd, diagnostic: buildDiagnostic({ run: error.run, usage: error.receipt?.usage, receiptPath: null, receiptStatus: error.receipt?.status }) });
  ...
  if (run.terminalReason === "provider_error") {
    ...
    const message = messages[attempt?.outcome] ?? "provider request failed.";
