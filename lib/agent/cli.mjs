...
export async function runAgentCli({ taskParts, cwd = process.cwd(), oafRoot = resolve(import.meta.dirname, "..", ".."), env = process.env, output = console } = {}) {
  ...
  if (run.terminalReason === "provider_error") {
    ...
    const message = attempt?.outcome === "rate_limited" ? "HTTP 429 (rate limited)." : attempt?.outcome === "timeout" ? "request timed out." : attempt?.outcome === "invalid_json" ? "provider returned invalid JSON." : attempt?.httpStatus ? `HTTP ${attempt.httpStatus}.` : "provider request failed.";
