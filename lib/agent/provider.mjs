...
export function normalizeProviderAttempt(attempt, fallbackTurn = 1) {
  const value = /** @type {any} */ (isPlainObject(attempt) ? attempt : {});
  const turn = Number.isSafeInteger(value.turn) && value.turn >= 1 ? value.turn : fallbackTurn;
  const durationMs = Number.isSafeInteger(value.durationMs) && value.durationMs >= 0 ? value.durationMs : 0;
  const outcome = value.outcome === "success" || VALID_PROVIDER_OUTCOMES.has(value.outcome) ? value.outcome : "unknown_provider_error";
  ...
  if (HTTP_FAILURE_OUTCOME_SET.has(outcome)) {
