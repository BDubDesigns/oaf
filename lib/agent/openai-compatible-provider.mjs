  ...
  for (const message of request.messages) {
    ...
    if (message.role === "tool") {
      ...
      for (const result of message.toolResults) {
        ...
        const code = /** @type {import("./contracts.ts").ToolErrorCode} */ (Object.hasOwn(PUBLIC_TOOL_ERRORS, result.errorCode) ? result.errorCode : "TOOL_EXECUTION_FAILED");
...
export function createOpenAICompatibleProvider({
  ...
  return {
    async complete(request) {
      ...
      if (response.status < 200 || response.status > 299) {
        throw new ProviderFailure(outcomeFromStatus(response.status), { httpStatus: response.status });
      }
      ...
      try {
        ...
        throw new ProviderFailure("invalid_json");
      }
